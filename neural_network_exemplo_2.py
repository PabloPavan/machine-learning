...
def main():
    ...
    for num_instance in range(0, 1):
        ...
        print("Propagando entrada", list_values[0], list_values[1])
        ...
        for j in range(0, network[len(network)-1]):
            result_nn[pos] = list_values[network[0]+j]
            pos += 1
        ...
        np.matmul(l_weight_layers[0], instance_nn, temp_result_layer)

...
def main():
    ...
    while num <= kfold :
        ...
        #print(dict_inf_extracted)
        class_dict.append(dict_inf_extracted[2])
        class_dict.append(dict_inf_extracted[0])
        class_dict.append(dict_inf_extracted[1])
        #print(class_dict)
        #print(len(result_voting))
        #calcula matriz de confusao 3x3
        for j in range(0, len(result_voting)):
            ...
            if result_voting[j] == result_gt[j] and class_dict[2] == result_gt[j]:
                confusion_matrix[2][2] += 1

            if result_voting[j] != result_gt[j] and class_dict[0] == result_voting[j]:
                if result_gt[j] == class_dict[1]:
                    confusion_matrix[1][0] += 1
                if result_gt[j] == class_dict[2]:
                    confusion_matrix[2][0] += 1

            if result_voting[j] != result_gt[j] and class_dict[1] == result_voting[j]:
                if result_gt[j] == class_dict[0]:
                    confusion_matrix[0][1] += 1
                if result_gt[j] == class_dict[2]:
                    confusion_matrix[2][1] += 1

            if result_voting[j] != result_gt[j] and class_dict[2] == result_voting[j]:
                if result_gt[j] == class_dict[0]:
